...
import 'package:horosa/pages/liuren/pages.dart';
...
class LiuRenFormPage extends StatefulWidget {
  static String route = '/liuren/form';

  const LiuRenFormPage({super.key});

  @override
  State<LiuRenFormPage> createState() => _LiuRenFormPageState();
}

class _LiuRenFormPageState extends State<LiuRenFormPage> {
  ...
  late Calendar _ast;
  late Calendar _time;
  LayoutType _layoutType = LayoutType.horizontal;
  ...
  int? _stringToInt(String input) {
    try {
      // 尝试将字符串解析为整数
      return int.parse(input);
    } on FormatException catch (e) {
      // 捕获格式异常，如果字符串无法转换为整数
      Log.e('Error: Invalid format - ${e.message}');
      return null;
    } catch (e) {
      // 捕获其他可能的异常
  ...
  void initState() {
    super.initState();
    _layoutType = context.read<ConfigProvider>().layout;
    setState(() {
      _time = Calendar(
          type: CalendarType.solar, value: Solar.fromDate(DateTime.now()));
      _ast =
        ...
        child: Padding(
          ...
          child: Column(
            children: [
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        HorosaRadio(
                          label: LayoutType.horizontal.label,
                          value: LayoutType.horizontal,
                          checked:
                              _layoutType.value == LayoutType.horizontal.value,
                          onChange: (val) {
                            setState(() {
                              _layoutType = val;
                            });
                        ...
                        SizedBox(width: 36.w),
                        HorosaRadio(
                          label: LayoutType.grid.label,
                          value: LayoutType.grid,
                          checked: _layoutType.value == LayoutType.grid.value,
                          onChange: (val) {
                            setState(() {
                              _layoutType = val;
                            });
              ...
              GestureDetector(
                onTap: () async {
                  // Navigator.of(context).push(MaterialPageRoute(builder: (context) => const LiuRenResultPage()));
                  LiuRenInput input = LiuRenInput(
                    guaType: _layoutType.value,
                    ...
                    hourNum: _stringToInt(_numController.text),
                    month: _ast.value.getLunar().getMonthInChinese(),
