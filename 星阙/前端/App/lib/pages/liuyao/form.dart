...
import 'package:horosa/pages/liuyao/pages.dart';
import 'package:horosa/pages/liuyao/widgets/app_bar_title.dart';
...
import 'widgets/lines_start_running.dart';

class LiuYaoFormPage extends StatefulWidget {
  static String route = '/liuyao/form';

  const LiuYaoFormPage({super.key});

  @override
  State<LiuYaoFormPage> createState() => _LiuYaoFormPageState();
}

class _LiuYaoFormPageState extends State<LiuYaoFormPage> {
  final TextEditingController _controller = TextEditingController(text: '');
  late List<LineTypes?> _lines;
  ...
  late Calendar _ast;
  late Calendar _time;
  final ValueNotifier<bool> _changed = ValueNotifier<bool>(true);
  ...
  Solar _calculateAST(Solar? solar) {
    ...
    //     astYear, astMonth, astDay, astHour, astMinute, astSecond);
    return solar;
  }
  ...
  void _reset() {
    ...
    setState(() {
      _controller.text = '';
      _lines = [];
      _startType = StartTypes.specified;
      _countryId = 45;
      _timezone = '亚洲/北京';
      _countryCNName = '中国';
      _place = '默认地点 北京';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          ...
          centerTitle: true,
          title: const LiuYaoAppBarTitle()),
      body: SingleChildScrollView(
        child: Padding(
          padding: EdgeInsets.fromLTRB(36.w, 0, 36.w, 36.w),
          child: Column(
            children: [
              Container(
              ...
              LinesStartRunning(
                changed: _changed,
                onChange: (linesAndStartType) {
                  WidgetsBinding.instance.addPostFrameCallback((_) {
                    setState(() {
                      _lines = linesAndStartType.lines;
                      _startType = linesAndStartType.startType;
                    });
                  });
                },
              ),
              GestureDetector(
                onTap: () async {
                  if (_startType.type == 1) {
                    if (_lines.fold(false, (a, b) => a || (b?.line == null))) {
                      toast('卦不能为空！');
                      return;
                    }
                  }
                  LiuYaoInput input = LiuYaoInput(
                    guaType: _startType.type,
                    question: _controller.text,
                    country: _countryCNName,
                    address: _place,
                    lines: _lines.reversed.fold('', (a, b) => a + b!.line),
                    jieqi: _ast.value.getLunar().getPrevJieQi(true).getName(),
                    hseb: Hseb(
                      year: _ast.value.getLunar().getYearInGanZhi(),
                      month: _ast.value.getLunar().getMonthInGanZhi(),
                      day: _ast.value.getLunar().getDayInGanZhi(),
                      hour: _ast.value.getLunar().getTimeInGanZhi(),
                    ),
