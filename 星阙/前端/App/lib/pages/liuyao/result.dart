...
import 'widgets/plum_blossom.dart';
import 'widgets/changed_hexagram.dart';
import 'widgets/non_changed_hexagram.dart';
...
enum _ResultType {
  ...
  const _ResultType(this.name, this.to, this.value, this.type);
}

class LiuYaoResultPage extends StatefulWidget {
  static const String route = '/liuyao/result';

  const LiuYaoResultPage({super.key});

  @override
  State<LiuYaoResultPage> createState() => _LiuYaoResultPageState();
}

class _LiuYaoResultPageState extends State<LiuYaoResultPage> {
  final ScreenshotController screenshotController = ScreenshotController();
  bool loading = true;
  late TextEditingController controller;
  _ResultType type = _ResultType.line;
  Plum? plum;
  Gender? gender; // 卦主性别
  String? zodiac; // 卦主生肖
  String xingNian = '';
  ...
  late LiuYaoInput input;
  late LiuYaoResult result;
  late LiuYaoExtras extras;
    ...
    return Screenshot(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          centerTitle: true,
          title: LiuYaoResultAppBar(input: input),
          actions: [
            GestureDetector(
              onTap: () async {
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          GestureDetector(
                            onTap: () {
                              setState(() {
                                type = type == _ResultType.line
                                    ? _ResultType.plum
                                    : _ResultType.line;
                                if (plum == null) {
                                  if (_ResultType.plum == type) {
                                    LiuYaoInput i = LiuYaoInput(
                                      address: input.address,
                                      country: input.country,
                                      guaTime: input.guaTime,
                                      guaType: 3,
                                      hseb: input.hseb,
                                      lines: result.gua.value,
                                      jieqi: input.jieqi,
                                      question: input.question,
                                      isSave: 2,
                                    );
                                    SixLineSvc.getSixLinesResult(i).then((res) {
                                      if (res.statusCode == 200) {
                                        if (res.data['code'] == 0) {
                                          setState(() {
                                            plum =
                                                Plum.fromJson(res.data['data']);
                                            item.extras.plum = plum;
                                            () async {
                                              try {
